 ...
 *
 * - Bugs with `status === 'fighting'` keep their status across rescans.
 * - Bugs with `status === 'defeated'` are never in the active list (they live in `defeatedBugs`).
 * - Untouched or newly spawned bugs get the fresh snapshot.
 */
export function mergeSpawnedBugs(previous: Bug[], spawned: Bug[]): Bug[] {
  ...
  return spawned.map((s) => {
    ...
    if (p.status === 'fighting') {
      return { ...s, status: 'fighting' as const };
    }
    return s;
  });
}
